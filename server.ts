...
const SCHEDULED_POSTS_PATH = path.join(process.cwd(), "scheduled-posts.json");
  ...
  app.post("/api/instagram/login-manual", async (req, res) => {
    ...
    try {
      ...
      console.log(`[Instagram] Token manual validado — ${resp.data.name}`);
    } catch (e: any) {
      return res.status(400).json({ error: "Token inválido", detail: e.response?.data?.error?.message || e.message });
    }
    ...
    res.json({ success: true, message: "Token do Instagram salvo!" });
  });
  ...
  // Resolve Buffer organization ID (cached)
