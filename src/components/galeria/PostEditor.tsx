...
export default function PostEditor({ posts, initialIndex = 0, onClose, onDeletePost, onUpdatePost, allPosts, onBufferUpdate }: any) {
  ...
  const fetchScheduleTimes = async (profileId: string) => {
    try {
      const response = await fetch(`https://wrybqqitsylqyhgzodyc.supabase.co/functions/v1/buffer/schedule/${profileId}`);
  ...
  const fetchIntegrations = async () => {
    try {
      const [igResp, bufferResp] = await Promise.all([
        fetch("https://wrybqqitsylqyhgzodyc.supabase.co/functions/v1/instagram/me"),
        fetch("https://wrybqqitsylqyhgzodyc.supabase.co/functions/v1/buffer/profiles")
      ]);
